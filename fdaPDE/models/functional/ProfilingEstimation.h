...
namespace fdaPDE
{
  namespace models
  {
    ...
    template <typename Model, typename ImplType>
    class ProfilingEstimationImpl;
    ...
    struct missing_data
    {
    }; // specialized implementation for the missing data setting
    ...
    public:
      ProfilingEstimation(const Model &m, double tol, std::size_t max_iter)
      {
        if (m.has_nan()) // missing data
          pe_ = std::make_unique<
              ProfilingEstimationImpl<Model_, missing_data>>(m, tol, max_iter);
        else // fallback to complete data setting
          pe_ = std::make_unique<
              ProfilingEstimationImpl<Model_, complete_data>>(m, tol, max_iter);
      }
    ...
    public:
      ...
      ProfilingEstimationStrategy(double tol, std::size_t max_iter) : tol_(tol), max_iter_(max_iter){};
    ...
    public:
      using type = typename std::conditional<
          ...
          SRPDE<PDE, sampling>,
          STRPDE<PDE, fdaPDE::models::SpaceTimeSeparable, sampling, fdaPDE::models::MonolithicSolver>>::type;
    };
    ...
    public:
      ...
      virtual void compute(const BlockFrame<double, int> &df, const SVector<model_traits<Model_>::n_lambda> &lambda)
      {
        ...
        while (!almost_equal(Jnew, Jold, this->tol_) && this->k_ < this->max_iter_)
        {
          ...
          g_ = solution.bottomRows(N);   // estimated field and PDE misfit
