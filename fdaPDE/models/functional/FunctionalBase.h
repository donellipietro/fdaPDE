...
namespace fdaPDE
{
  namespace models
  {
    ...
    protected:
      ...
      std::vector<SpMatrix<double>> PsiNaN_;                  // matrix \Psi of the i-th statistical unit where rows corresponding to NaN observations are zeroed
    public:
      ...
      template <typename U = Model, // fake type to enable substitution in SFINAE
                typename std::enable_if<!std::is_same<typename model_traits<U>::regularization, SpaceOnly>::value,
                                        int>::type = 0>
      ...
      void init_nan()
      {
        ...
        for (std::size_t i = 0; i < n_stat_units(); ++i)
        {
          ...
          if (!nan_idxs_[i].empty())
          {
            for (std::size_t i = 0; i < n_stat_units(); ++i)
            {
              PsiNaN_[i].resize(n, N); // reserve space
              ...
              PsiNaN_[i].setFromTriplets(tripletList.begin(), tripletList.end());
              PsiNaN_[i].makeCompressed();
            }
          }
      ...
      const SpMatrix<double> &Psi(std::size_t i) const { return has_nan(i) ? PsiNaN_[i] : Psi(not_nan()); }
      auto PsiTD(std::size_t i) const { return has_nan(i) ? PsiNaN_[i].transpose() * D() : Psi(not_nan()).transpose() * D(); }
