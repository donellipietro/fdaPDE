#ifndef __FPLSR_SIMPLS_H__
#define __FPLSR_SIMPLS_H__
...
namespace fdaPDE
{
    namespace models
    {
        ...
        class FPLSR_SIMPLS : public FunctionalRegressionBase<FPLSR_SIMPLS<PDE, RegularizationType, SamplingDesign, lambda_selection_strategy>>
        {
            // compile time checks
            static_assert(std::is_base_of<PDEBase, PDE>::value);

        private:
            typedef FPLSR_SIMPLS<PDE, RegularizationType, SamplingDesign, lambda_selection_strategy> ModelType;
            ...
            SpMatrix<double> R_term_{};

            // energy matrix
            SpMatrix<double> A_{};

            // problem solution
            DMatrix<double> R_; // directions in X-spce
            DMatrix<double> Q_; // directions in Y-space
            DMatrix<double> T_; // X-latent components
            DMatrix<double> U_; // Y-latent components
            DMatrix<double> P_; // X loadings
            DMatrix<double> V_; // Orthonormal basis of P columnspace

            // analysis results
            DVector<double> h_;
            double varX_;
            double varY_;
            DVector<double> varX_explained_;
            DVector<double> varY_explained_;
        ...
        public:
            ...
            {
                return (T() * P().transpose()).rowwise() + this->X_mean().transpose();
            }

            virtual ~FPLSR_SIMPLS() = default;
        };
        template <typename PDE_, typename RegularizationType_,
                  typename SamplingDesign_, typename lambda_selection_strategy>
        struct model_traits<FPLSR_SIMPLS<PDE_, RegularizationType_, SamplingDesign_, lambda_selection_strategy>>
        {
            typedef PDE_ PDE;
        ...
        struct model_traits<FPLSR_SIMPLS<PDE_, fdaPDE::models::SpaceTimeSeparable, SamplingDesign, lambda_selection_strategy>>
        {
            typedef PDE_ PDE;
...
#include "fPLSR_SIMPLS.tpp"

    }
}

#endif // __FPLSR_SIMPLS_H__
