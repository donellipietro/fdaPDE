...
TEST(FPLSR, Test0_Laplacian_GeostatisticalAtNodes_comparison_with_harold)
{
  ...
  for (unsigned int i : tests)
  {
    ...
    file = reader.parseFile(test_directory + "Y_mean.csv");
    ...
    file = reader.parseFile(test_directory + "X_hat.csv");
    ...
    file = reader.parseFile(test_directory + "X_mean.csv");
    ...
    file = reader.parseFile(test_directory + "V.csv");
    ...
    if (VERBOSE)
    {
      ...
      std::cout << "Error norm: " << (ns(expected_V) - ns(V)).lpNorm<Eigen::Infinity>() << std::endl;
      std::cout << "----------------" << std::endl;
      std::cout << std::endl;
    }
