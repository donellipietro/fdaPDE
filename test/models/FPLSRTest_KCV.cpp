...
namespace Test_fPLSR_GCV
{
                                 ...
                                 std::vector<double> &errors_B)
    {
        ...
        outfile << Y_hat.format(Test_fPLSR_GCV::CSVFormat);
        outfile.close();
        ...
        outfile << X_hat.format(Test_fPLSR_GCV::CSVFormat);
        outfile.close();
        ...
        outfile << B_hat.format(Test_fPLSR_GCV::CSVFormat);
        outfile.close();
    }
...
/* test 1:
   ...
   BC:           no
   order FE:     1
   lambda selection: KCV
 */
TEST(FPLSR, Test1_Laplacian_GeostatisticalAtNodes_KCV)
{
    ...
    for (unsigned int i : tests)
    {
        ...
        std::string results_directory = test_directory + "results_KCV/";
        ...
        Test_fPLSR_GCV::compareAndExportResults(test_directory,
                                                results_directory,
                                                VERBOSE,
                                                Y_hat,
                                                X_hat,
    ...
    Test_fPLSR_GCV::exportErrors(tests_directory,
                                 tests,
                                 VERBOSE,
                                 errors_Y,
                                 errors_X,
