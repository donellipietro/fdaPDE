...
namespace Test_fPLSR_SIMPLS_comparison
{
...
/* test comparison:
   ...
   order FE:     1
 */
TEST(FPLSR_SIMPLS, Test_comparison_Laplacian_GeostatisticalAtNodes)
{
    ...
    // Tests
    std::string tests_directory = "data/models/FPLSR_SIMPLS/2D_test_comparison/";
    bool VERBOSE = true;
    std::vector<unsigned int> tests{1, 2, 3, 4, 5, 6};
    unsigned int n_batches = 2;
    ...
    // tests options
    bool TEST = true;
    std::vector<std::string> test_name_vect = {"ns_l0", "ns", "sr", "sri"};
    ...
    for (unsigned int i : tests)
    {
        ...
        for (unsigned int j = 1; j <= n_batches; ++j)
        {
            ...
            for (unsigned int t = 0; t < test_name_vect.size(); ++t)
            {
                ...
                // outfile << Y_hat.format(Test_fPLSR_SIMPLS_comparison::CSVFormat);
                // outfile.close();

                // outfile.open(results_directory + "B_hat_" + test_name + "_" + std::to_string(j) + ".csv");
                // outfile << B_hat.format(Test_fPLSR_SIMPLS_comparison::CSVFormat);
                // outfile.close();

                // outfile.open(results_directory + "X_hat_" + test_name + "_" + std::to_string(j) + ".csv");
                // outfile << X_hat.format(Test_fPLSR_SIMPLS_comparison::CSVFormat);
                // outfile.close();
            }
            std::cout << std::endl;
        }
    ...
    outfile << errors_Y.format(Test_fPLSR_SIMPLS_comparison::CSVFormat);
    outfile.close();
    outfile.open(tests_directory + "errors_Y_train.csv");
    outfile << errors_Y_train.format(Test_fPLSR_SIMPLS_comparison::CSVFormat);
    outfile.close();

    outfile.open(tests_directory + "errors_X.csv");
    outfile << errors_X.format(Test_fPLSR_SIMPLS_comparison::CSVFormat);
    outfile.close();
    outfile.open(tests_directory + "errors_X_train.csv");
    outfile << errors_X_train.format(Test_fPLSR_SIMPLS_comparison::CSVFormat);
    outfile.close();

    outfile.open(tests_directory + "errors_B.csv");
    outfile << errors_B.format(Test_fPLSR_SIMPLS_comparison::CSVFormat);
    outfile.close();
}